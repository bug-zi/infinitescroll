...
export type PaperBorderDriftResult = {
  hasPaperBorderDrift: boolean;
  reason?: string;
};
...
export async function detectPaperBorderDrift(imageBytes: Buffer | Uint8Array): Promise<PaperBorderDriftResult> {
  ...
  const topPaper = isPalePaperLike(top, middle);
  const bottomPaper = isPalePaperLike(bottom, middle);
  if (topPaper && bottomPaper) {
    return {
...
function getBandStats(data: Buffer, width: number, height: number, top: number, bandHeight: number): BandStats {
...
function isPalePaperLike(band: BandStats, middle: BandStats) {
